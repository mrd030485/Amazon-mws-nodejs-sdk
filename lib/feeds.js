...
module.exports = {
    ...
    CancelFeedSubmissions: function (config, params, xml, cb) {
        ...
        if (_.has(params, 'FeedTypeList')) {
            var feedListNum = 1;
        _.each(params['FeedTypeList'], function(type){
            parameters['FeedTypeList.Type.'+feedListNum] = type;
        });
        }

        if (_.has(params, 'FeedSubmissionIdList')) {
            var feedSubmissionIdNum = 1;
            _.each(params['FeedSubmissionIdList'], function(id){
                parameters['FeedSubmissionIdList.Id.'+feedSubmissionIdNum] = id;
            });
        }
    ...
    GetFeedSubmissionList: function (config, params, xml, cb) {
        ...
        if (_.has(params, 'FeedProcessingStatusList')) {
            var feedStatusNum = 1;
            _.each(params['FeedProcessingStatusList'], function(status){
                if(_.has(this.FeedProcessingStatus, status)){
                    parameters['FeedProcessingStatusList.Status.'+feedStatusNum] = status;
                }
            });
        }
    ...
    GetFeedSubmissionListByNextToken: function (config, params, xml, cb) {
        ...
        if (!_.has(params, 'NextToken')) {
            defer.reject({'Error': 'You need to pass parameter NextToken'
            }, null);
            return;
        }
        var parameters = {
    ...
    GetFeedSubmissionCount: function (config, params, xml, cb) {
        ...
        if (_.has(params, 'FeedTypeList')) {
            var feedListNum = 1;
            _.each(params['FeedTypeList'], function(type){
                if(_.contains(this.FeedTypes, type)){
                    parameters['FeedTypeList.Type.'+feedListNum] = type;
                }
            });
        }
    ...
    GetFeedSubmissionResult: function (config, params, xml, cb) {
        ...
        if(!_.has(params,'FeedSubmissionId')){
            defer.reject({'Error' : 'You need to pass parameter FeedSubmissionId'
            }, null);
            return;
        }
        var parameters = {
    ...
    SubmitFeed: function (config, params, xml, cb) {
        ...
        if (!_.has(params, 'FeedType')) {
            defer.reject({'Error': 'You need to pass parameter FeedType'
            }, null);
            return;
        }
        if (!_.contains(this.FeedTypes, params['FeedType'])) {
            defer.reject({'Error': 'FeedType must be one of the FeedTypes listed in AmazonDocs'
            }, null);
            return;
        }
