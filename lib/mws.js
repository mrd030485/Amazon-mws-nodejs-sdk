...
module.exports = {
    ...
    createRequest: function (params, cb) {
        ...
        if(params.type === 'Reports'){
            _.extend((paramObj, {'Merchant': params.config.SellerId}));   
        }else{
            _.extend((paramObj, {'SellerId': params.config.SellerId}));
        }
        ...
        var options = {
            host: url,
            port: 443,
            path: endpoint[params.type].type + (params.type === 'Reports'? '' : endpoint[params.type].version) + "?" + body,
            method: "POST",
            headers: headers
        };
    ...
    generateSignature: function (config, params, ep) {
        ...
        var queryRequest = ['POST', url, endpoint[ep].type + (ep === 'Reports'? '' : endpoint[ep].version), strParams].join('\n');
