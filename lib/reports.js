...
module.exports = {
    ...
    GetReport: function (config, params, xml, cb) {
        ...
        if (!_.has(params, 'ReportId')) {
            defer.reject({Error: "You must provide ReportId parameter"});
        }
        var parameters = {
    ...
    GetReportCount: function (config, params, xml, cb) {
        ...
        if (_.has(params, 'ReportTypeList')) {
            var reportTypeNum = 1;
            _.each(params['ReportTypeList'], function(reportType){
               parameters['ReportTypeList.Type.'+reportTypeNum] = reportType;
            });
        }
    ...
    GetReportList: function (config, params, xml, cb) {
        ...
        if (_.has(params, 'ReportRequestIdList')) {
            var reportRequestIdNum = 1;
            _.each(params['ReportRequestIdList'], function(reportRequestId){
               parameters['ReportRequestIdList.Id.'+reportRequestIdNum] = reportRequestId;
            });
        }
        mws.createRequest({
    ...
    GetReportListByNextToken: function (config, params, xml, cb) {
        ...
        if (!_.has(params, 'NextToken')) {
            defer.reject({Error: "You need to pass NextToken parameter"});
        }
        var parameters = {
    ...
    GetReportRequestCount: function (config, params, xml, cb) {
        ...
        if (_.has(params, 'ReportProcessingStatusList')) {
            var idNum = 1;
            _.each(params['ReportProcessingStatusList'], function(reportStatus){
               parameters['ReportProcessingStatusList.Status.'+idNum] = reportStatus;
            });
        }
        if (_.has(params, 'ReportTypeList')) {
            var reportTypeNum = 1;
    ...
    RequestReport: function (config, params, xml, cb) {
        ...
        if (!_.has(params, 'ReportType')) {
            defer.reject({Error: "You need to provide ReportType parameter"});
        }
        var parameters = {
        ...
        if (_.has(params, 'MarketplaceIdList')) {
            var idNum = 1;
            _.each(params['MarketplaceIdList'], function(marketplaceId){
               parameters['MarketplaceIdList.Id.'+idNum] = marketplaceId;
            });
        }
        mws.createRequest({
    ...
    ManageReportSchedule: function (config, params, xml, cb) {
        ...
        if (!_.has(params, 'ReportType') || !_.has(params, 'Schedule')) {
            defer.reject({Error: "You need to provide parameters ReportType and Schedule"});
        }
        if (!_.contains(this.ReportTypeList, params['ReportType'])) {
            defer.reject({Error: "ReportType must be valid type."});
        }
        if (!_.contains(this.Schedules, params['Schedule'])) {
            defer.reject({Error: "Schedule must be valid Schedule value"});
        }
        var parameters = {
            'ReportType': params['ReportType'],
    ...
    UpdateReportAcknowledgements: function (config, params, xml, cb) {
        ...
        if (!_.has(params, 'ReportIdList')) {
            defer.reject({Error: "You need to provide ReportIdList parameter"});
        } else {
            var idNum = 1;
            _.each(params['ReportIdList'], function(reportId){
               parameters['ReportIdList.Id.'+idNum] = reportId;
            });
        }
        if (_.has(params, 'Acknowledged')) {
            parameters['Acknowledged'] = params['Acknowledged'];
